...
async def create_dictionary_ingredient(data: IngredientCreate, db: AsyncSession = Depends(get_db)):
    repo = MealIngredientsRepository(db)
    return await repo.create_ingredient(data.model_dump())

@router.get("/", response_model=List[IngredientRead])
async def list_dictionary_ingredients(db: AsyncSession = Depends(get_db)):
    repo = MealIngredientsRepository(db)
...
async def get_meal_ingredients(id_meal: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def remove_ingredient_from_recipe(id_recipe: uuid.UUID, db: AsyncSession = Depends(get_db)):
